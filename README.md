...
This is my solution to the free code camp machine learning Rock Paper Scissors project ( https://www.freecodecamp.org/learn/machine-learning-with-python/machine-learning-with-python-projects/rock-paper-scissors). The goal was to beat 4 bots with an accuracy of over 60%. I managed to have an average of 99% win rate against two of the bots, 85% and 65% for the other 2.

Strategy:
Instead of relying on a single algorithm, i built a multi-armed bandit ensemble. It uses 6 different agents, which predict the move that the opponet will play next.
Every round, the bot evaluates which of the 6 agents would have the best prediction rate in recent moves, using a decaying average (EMA)
